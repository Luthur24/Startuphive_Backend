...
# Flask + PostgreSQL + Cloudinary | All tables: sh_
...
JWT_SECRET    = "sh_v3_jwt_2025_xK9pLm"
JWT_ALGORITHM = "HS256"
...
def init_db():
    ...
    ddl = [
        ...
        # STARTUPS
        """CREATE TABLE IF NOT EXISTS sh_startups (
            id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
        ...
        # STARTUP CONTENT
        """CREATE TABLE IF NOT EXISTS sh_startup_content (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE UNIQUE,
        ...
        # METRICS
        """CREATE TABLE IF NOT EXISTS sh_metrics (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
        ...
        # ASKS
        """CREATE TABLE IF NOT EXISTS sh_asks (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
        ...
        """CREATE TABLE IF NOT EXISTS sh_team (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE SET NULL,
        ...
        # UPDATES / CHANGELOG
        """CREATE TABLE IF NOT EXISTS sh_updates (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE SET NULL,
        ...
        """CREATE TABLE IF NOT EXISTS sh_signals (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
        ...
        # REVIEW REPLIES
        """CREATE TABLE IF NOT EXISTS sh_review_replies (
            id SERIAL PRIMARY KEY,
            review_id INTEGER REFERENCES sh_reviews(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
            content TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",

        # UPVOTES
        """CREATE TABLE IF NOT EXISTS sh_upvotes (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        ...
        # FOLLOWS
        """CREATE TABLE IF NOT EXISTS sh_follows (
            id SERIAL PRIMARY KEY,
            startup_id INTEGER REFERENCES sh_startups(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        ...
        # COMMUNITY POSTS
        """CREATE TABLE IF NOT EXISTS sh_community_posts (
            id SERIAL PRIMARY KEY,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
        ...
        # COMMUNITY VOTES
        """CREATE TABLE IF NOT EXISTS sh_community_votes (
            id SERIAL PRIMARY KEY,
            post_id INTEGER REFERENCES sh_community_posts(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
        ...
        # COMMUNITY COMMENTS
        """CREATE TABLE IF NOT EXISTS sh_community_comments (
            id SERIAL PRIMARY KEY,
            post_id INTEGER REFERENCES sh_community_posts(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
            parent_id INTEGER REFERENCES sh_community_comments(id) ON DELETE CASCADE,
        ...
        """CREATE TABLE IF NOT EXISTS sh_conversation_members (
            id SERIAL PRIMARY KEY,
            conversation_id INTEGER REFERENCES sh_conversations(id) ON DELETE CASCADE,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
            last_read_at TIMESTAMP,
            UNIQUE(conversation_id, user_id))""",

        """CREATE TABLE IF NOT EXISTS sh_messages (
            id SERIAL PRIMARY KEY,
            conversation_id INTEGER REFERENCES sh_conversations(id) ON DELETE CASCADE,
            sender_id INTEGER REFERENCES sh_users(id) ON DELETE SET NULL,
        ...
        """CREATE TABLE IF NOT EXISTS sh_notifications (
            id SERIAL PRIMARY KEY,
            user_id INTEGER REFERENCES sh_users(id) ON DELETE CASCADE,
            actor_id INTEGER REFERENCES sh_users(id) ON DELETE SET NULL,
        ...
        # ACTIVITY
        """CREATE TABLE IF NOT EXISTS sh_activity (
    ...
    idx = [
        "CREATE INDEX IF NOT EXISTS idx3_s_slug      ON sh_startups(slug)",
        "CREATE INDEX IF NOT EXISTS idx3_s_user      ON sh_startups(user_id)",
        "CREATE INDEX IF NOT EXISTS idx3_s_active    ON sh_startups(is_active, updated_at DESC)",
        "CREATE INDEX IF NOT EXISTS idx3_s_upvotes   ON sh_startups(upvote_count DESC)",
        "CREATE INDEX IF NOT EXISTS idx3_uv_s        ON sh_upvotes(startup_id)",
        "CREATE INDEX IF NOT EXISTS idx3_uv_u        ON sh_upvotes(user_id)",
        "CREATE INDEX IF NOT EXISTS idx3_bk_u        ON sh_bookmarks(user_id)",
        "CREATE INDEX IF NOT EXISTS idx3_sig_s       ON sh_signals(startup_id)",
        "CREATE INDEX IF NOT EXISTS idx3_rev_s       ON sh_reviews(startup_id)",
        "CREATE INDEX IF NOT EXISTS idx3_cp_comm     ON sh_community_posts(community, created_at DESC)",
        "CREATE INDEX IF NOT EXISTS idx3_cv_post     ON sh_community_votes(post_id)",
        "CREATE INDEX IF NOT EXISTS idx3_cc_post     ON sh_community_comments(post_id)",
        "CREATE INDEX IF NOT EXISTS idx3_msg_conv    ON sh_messages(conversation_id, created_at)",
        "CREATE INDEX IF NOT EXISTS idx3_notif_u     ON sh_notifications(user_id, is_read)",
        "CREATE INDEX IF NOT EXISTS idx3_act_ts      ON sh_activity(created_at DESC)",
        "CREATE INDEX IF NOT EXISTS idx3_hist_s      ON sh_history(startup_id, changed_at DESC)",
        "CREATE INDEX IF NOT EXISTS idx3_upd_s       ON sh_updates(startup_id, created_at DESC)",
    ]
    for stmt in idx: cur.execute(stmt)
    # Migrations for existing DBs — add missing columns safely
    migrations = [
        "ALTER TABLE sh_startup_content ADD COLUMN IF NOT EXISTS video_url VARCHAR(500)",
        "ALTER TABLE sh_startup_content ADD COLUMN IF NOT EXISTS competitors TEXT[] DEFAULT '{}'",
    ]
    for m in migrations:
        try: cur.execute(m)
        except Exception: db.rollback()
    db.commit(); cur.close()
    print("✅ sh_ schema v3 ready — 22 tables")
...
def log_act(atype, uid=None, sid=None, meta=None):
    try:
        db = get_db(); cur = db.cursor()
        cur.execute("INSERT INTO sh_activity(type,user_id,startup_id,meta) VALUES(%s,%s,%s,%s)",
...
def notify(uid, actor_id, ntype, sid=None, pid=None, message="", link=""):
    ...
    try:
        db = get_db(); cur = db.cursor()
        cur.execute("""INSERT INTO sh_notifications
...
def register():
    ...
    db = get_db(); cur = db.cursor()
    cur.execute("SELECT id FROM sh_users WHERE email=%s OR username=%s", (email, username))
    if cur.fetchone(): cur.close(); return jsonify({"error":"Email or username taken"}), 409
    pw_hash = bcrypt.hashpw(pw.encode(), bcrypt.gensalt()).decode()
    cur.execute("INSERT INTO sh_users(name,username,email,password_hash) VALUES(%s,%s,%s,%s) RETURNING id",
...
def login():
    ...
    cur.execute("""SELECT id,name,username,email,password_hash,avatar_url,bio,headline,
                   ...
                   FROM sh_users WHERE email=%s OR username=%s""", (identifier, identifier))
    u = cur.fetchone(); cur.close()
    if not u or not bcrypt.checkpw(pw.encode(), u["password_hash"].encode()):
        return jsonify({"error":"Invalid credentials"}), 401
    c = get_db().cursor()
    c.execute("UPDATE sh_users SET last_active=NOW() WHERE id=%s", (u["id"],))
...
def get_me():
    ...
    cur.execute("""SELECT id,name,username,email,avatar_url,cover_url,bio,headline,contact_methods,
                   ...
                   FROM sh_users WHERE id=%s""", (g.uid,))
    u = cur.fetchone()
    if not u: cur.close(); return jsonify({"error":"Not found"}), 404
    cur.execute("SELECT COUNT(*) AS c FROM sh_startups WHERE user_id=%s AND is_active=TRUE", (g.uid,))
...
def update_profile():
    ...
    cur.execute(f"UPDATE sh_users SET {sets},updated_at=NOW() WHERE id=%s", list(updates.values())+[g.uid])
...
def change_password():
    ...
    db = get_db(); cur = db.cursor()
    cur.execute("SELECT password_hash FROM sh_users WHERE id=%s", (g.uid,))
    u = cur.fetchone()
    ...
    cur.execute("UPDATE sh_users SET password_hash=%s WHERE id=%s", (new_hash, g.uid))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def marquee():
    db = get_db(); cur = db.cursor()
    cur.execute("SELECT id,name,slug,verification_level FROM sh_startups WHERE is_active=TRUE ORDER BY updated_at DESC LIMIT 50")
...
def list_startups():
    ...
    need_join=""
    if need: need_join="JOIN sh_asks ak ON ak.startup_id=s.id AND ak.is_active=TRUE AND ak.type=%s"; params.insert(0,need)
    ...
    cur.execute(f"""SELECT s.*,u.name AS owner_name,u.username AS owner_username,u.avatar_url AS owner_avatar
                    FROM sh_startups s {need_join} LEFT JOIN sh_users u ON s.user_id=u.id
                    {where} ORDER BY {order} LIMIT %s OFFSET %s""",
                ([need]+params[1:] if need else params)+[per_page,offset])
    rows=cur.fetchall()
    cur.execute(f"SELECT COUNT(*) AS t FROM sh_startups s {need_join} {where}",
                [need]+params[1:] if need else params)
    total=cur.fetchone()["t"]; uv=bk=set()
    if g.uid and rows:
        ids=[r["id"] for r in rows]
        cur.execute("SELECT startup_id FROM sh_upvotes   WHERE user_id=%s AND startup_id=ANY(%s)",(g.uid,ids)); uv={r["startup_id"] for r in cur.fetchall()}
        cur.execute("SELECT startup_id FROM sh_bookmarks WHERE user_id=%s AND startup_id=ANY(%s)",(g.uid,ids)); bk={r["startup_id"] for r in cur.fetchall()}
    cur.close()
...
def get_startup(slug):
    ...
    cur.execute("""SELECT s.*,u.name AS owner_name,u.username AS owner_username,
                   ...
                   FROM sh_startups s LEFT JOIN sh_users u ON s.user_id=u.id
                   WHERE s.slug=%s""", (slug,))
    s=cur.fetchone()
    if not s: cur.close(); return jsonify({"error":"Not found"}), 404
    cur.execute("UPDATE sh_startups SET view_count=view_count+1 WHERE id=%s",(s["id"],)); get_db().commit()
    uv=bk=fw=False
    if g.uid:
        cur.execute("SELECT 1 FROM sh_upvotes   WHERE startup_id=%s AND user_id=%s",(s["id"],g.uid)); uv=bool(cur.fetchone())
        cur.execute("SELECT 1 FROM sh_bookmarks WHERE startup_id=%s AND user_id=%s",(s["id"],g.uid)); bk=bool(cur.fetchone())
        cur.execute("SELECT 1 FROM sh_follows   WHERE startup_id=%s AND user_id=%s",(s["id"],g.uid)); fw=bool(cur.fetchone())
    cur.execute("SELECT * FROM sh_startup_content WHERE startup_id=%s",(s["id"],)); content=cur.fetchone()
    cur.execute("SELECT * FROM sh_metrics WHERE startup_id=%s AND is_current=TRUE ORDER BY recorded_at DESC",(s["id"],)); metrics=cur.fetchall()
    cur.execute("SELECT * FROM sh_asks WHERE startup_id=%s AND is_active=TRUE ORDER BY urgency DESC,created_at",(s["id"],)); asks=cur.fetchall()
    cur.execute("SELECT * FROM sh_team WHERE startup_id=%s ORDER BY sort_order,id",(s["id"],)); team=cur.fetchall()
    cur.execute("SELECT * FROM sh_updates WHERE startup_id=%s ORDER BY is_pinned DESC,created_at DESC LIMIT 20",(s["id"],)); updates=cur.fetchall()
    cur.execute("SELECT * FROM sh_history WHERE startup_id=%s ORDER BY changed_at DESC LIMIT 2",(s["id"],)); history=cur.fetchall()
    cur.execute("SELECT signal_type,COUNT(*) AS c FROM sh_signals WHERE startup_id=%s AND is_active=TRUE GROUP BY signal_type",(s["id"],)); sig_counts={r["signal_type"]:r["c"] for r in cur.fetchall()}
    cur.execute("SELECT COUNT(*) AS c FROM sh_follows WHERE startup_id=%s",(s["id"],)); follower_count=cur.fetchone()["c"]
    cur.execute("SELECT AVG(rating) AS avg,COUNT(*) AS cnt FROM sh_reviews WHERE startup_id=%s",(s["id"],)); rev=cur.fetchone()
    cur.close()
...
def create_startup():
    ...
    while True:
        cur.execute("SELECT id FROM sh_startups WHERE slug=%s",(slug,))
    ...
    cur.execute("""INSERT INTO sh_startups
    ...
    cur.execute("""INSERT INTO sh_startup_content
...
def update_startup(sid):
    db=get_db(); cur=db.cursor()
    cur.execute("SELECT user_id FROM sh_startups WHERE id=%s",(sid,)); row=cur.fetchone()
    ...
    if updates:
        for field in ["stage","status","team_size"]:
            if field in updates:
                cur.execute(f"SELECT {field} FROM sh_startups WHERE id=%s",(sid,))
                old=cur.fetchone()
                if old: cur.execute("INSERT INTO sh_history(startup_id,user_id,field_name,old_value,new_value) VALUES(%s,%s,%s,%s,%s)",(sid,g.uid,field,str(old[field] or ""),str(updates[field] or "")))
        sets=", ".join(f"{k}=%s" for k in updates)
        cur.execute(f"UPDATE sh_startups SET {sets},updated_at=NOW() WHERE id=%s",list(updates.values())+[sid])
    c=d.get("content")
    if c:
        ...
        if cu:
            cur.execute("SELECT id FROM sh_startup_content WHERE startup_id=%s",(sid,))
            if cur.fetchone():
                s2=", ".join(f"{k}=%s" for k in cu)
                cur.execute(f"UPDATE sh_startup_content SET {s2},updated_at=NOW() WHERE startup_id=%s",list(cu.values())+[sid])
            else:
                cur.execute("INSERT INTO sh_startup_content(startup_id) VALUES(%s)",(sid,))
                s2=", ".join(f"{k}=%s" for k in cu)
                cur.execute(f"UPDATE sh_startup_content SET {s2} WHERE startup_id=%s",list(cu.values())+[sid])
    db.commit(); cur.close(); return jsonify({"success":True})
...
def delete_startup(sid):
    ...
    if not row: cur.close(); return jsonify({"error":"Not found"}), 404
    if row["user_id"]!=g.uid: cur.close(); return jsonify({"error":"Forbidden"}), 403
    cur.execute("UPDATE sh_startups SET is_active=FALSE WHERE id=%s",(sid,))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def upvote(sid):
    db=get_db(); cur=db.cursor()
    try:
        cur.execute("INSERT INTO sh_upvotes(startup_id,user_id) VALUES(%s,%s)",(sid,g.uid))
        cur.execute("UPDATE sh_startups SET upvote_count=upvote_count+1,signal_score=signal_score+2 WHERE id=%s",(sid,))
        cur.execute("UPDATE sh_users SET reputation=reputation+1 WHERE id=(SELECT user_id FROM sh_startups WHERE id=%s)",(sid,))
        db.commit(); cur.close(); log_act("upvote",g.uid,sid); return jsonify({"upvoted":True})
    except psycopg2.errors.UniqueViolation:
        db.rollback()
        cur.execute("DELETE FROM sh_upvotes WHERE startup_id=%s AND user_id=%s",(sid,g.uid))
        cur.execute("UPDATE sh_startups SET upvote_count=GREATEST(0,upvote_count-1),signal_score=GREATEST(0,signal_score-2) WHERE id=%s",(sid,))
        cur.execute("UPDATE sh_users SET reputation=GREATEST(0,reputation-1) WHERE id=(SELECT user_id FROM sh_startups WHERE id=%s)",(sid,))
...
def bookmark(sid):
    db=get_db(); cur=db.cursor()
    try:
        cur.execute("INSERT INTO sh_bookmarks(startup_id,user_id) VALUES(%s,%s)",(sid,g.uid))
        cur.execute("UPDATE sh_startups SET bookmark_count=bookmark_count+1 WHERE id=%s",(sid,))
        db.commit(); cur.close(); return jsonify({"bookmarked":True})
    except psycopg2.errors.UniqueViolation:
        db.rollback()
        cur.execute("DELETE FROM sh_bookmarks WHERE startup_id=%s AND user_id=%s",(sid,g.uid))
        cur.execute("UPDATE sh_startups SET bookmark_count=GREATEST(0,bookmark_count-1) WHERE id=%s",(sid,))
...
def my_bookmarks():
    ...
    cur.execute("""SELECT s.*,u.name AS owner_name,u.username AS owner_username,u.avatar_url AS owner_avatar
                   FROM sh_startups s JOIN sh_bookmarks b ON b.startup_id=s.id
                   LEFT JOIN sh_users u ON s.user_id=u.id WHERE b.user_id=%s ORDER BY b.created_at DESC""",(g.uid,))
...
def follow_startup(sid):
    db=get_db(); cur=db.cursor()
    try:
        cur.execute("INSERT INTO sh_follows(startup_id,user_id) VALUES(%s,%s)",(sid,g.uid))
        cur.execute("UPDATE sh_startups SET signal_score=signal_score+1 WHERE id=%s",(sid,))
        db.commit(); cur.close(); return jsonify({"following":True})
    except psycopg2.errors.UniqueViolation:
        db.rollback()
        cur.execute("DELETE FROM sh_follows WHERE startup_id=%s AND user_id=%s",(sid,g.uid))
        cur.execute("UPDATE sh_startups SET signal_score=GREATEST(0,signal_score-1) WHERE id=%s",(sid,))
...
def signal_startup(sid):
    ...
    db=get_db(); cur=db.cursor()
    try:
        cur.execute("INSERT INTO sh_signals(startup_id,user_id,signal_type,amount,currency,message) VALUES(%s,%s,%s,%s,%s,%s)",
                    (sid,g.uid,stype,d.get("amount"),d.get("currency","USD"),d.get("message","")))
        cur.execute("UPDATE sh_startups SET signal_score=signal_score+3 WHERE id=%s",(sid,))
        db.commit()
        cur.execute("SELECT user_id FROM sh_startups WHERE id=%s",(sid,))
        owner=cur.fetchone()
        ...
        db.rollback()
        cur.execute("DELETE FROM sh_signals WHERE startup_id=%s AND user_id=%s AND signal_type=%s",(sid,g.uid,stype))
        cur.execute("UPDATE sh_startups SET signal_score=GREATEST(0,signal_score-3) WHERE id=%s",(sid,))
...
def get_signals(sid):
    ...
    cur.execute("""SELECT s.*,u.name AS user_name,u.username AS user_username,u.avatar_url AS user_avatar
                   FROM sh_signals s JOIN sh_users u ON s.user_id=u.id
...
def get_reviews(sid):
    ...
    cur.execute("""SELECT r.*,u.name AS reviewer_name,u.username AS reviewer_username,u.avatar_url AS reviewer_avatar
                   FROM sh_reviews r JOIN sh_users u ON r.user_id=u.id
    ...
    for rv in reviews:
        cur.execute("""SELECT rp.*,u.name AS replier_name,u.avatar_url AS replier_avatar
                       FROM sh_review_replies rp JOIN sh_users u ON rp.user_id=u.id
...
def post_review(sid):
    ...
    db=get_db(); cur=db.cursor()
    try:
        cur.execute("INSERT INTO sh_reviews(startup_id,user_id,rating,title,content) VALUES(%s,%s,%s,%s,%s)",
...
def reply_review(rid):
    ...
    db=get_db(); cur=db.cursor()
    cur.execute("INSERT INTO sh_review_replies(review_id,user_id,content) VALUES(%s,%s,%s) RETURNING id",
                (rid,g.uid,content))
...
def add_ask(sid):
    ...
    cur.execute("INSERT INTO sh_asks(startup_id,type,description,urgency) VALUES(%s,%s,%s,%s) RETURNING id",
...
def respond_ask(aid):
    db=get_db(); cur=db.cursor()
    cur.execute("UPDATE sh_asks SET response_count=response_count+1 WHERE id=%s",(aid,))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def add_metric(sid):
    ...
    if not row or row["user_id"]!=g.uid: cur.close(); return jsonify({"error":"Forbidden"}), 403
    d=request.get_json() or {}
    cur.execute("INSERT INTO sh_metrics(startup_id,label,value,confidence) VALUES(%s,%s,%s,%s) RETURNING id",
...
def delete_metric(mid):
    db=get_db(); cur=db.cursor()
    cur.execute("DELETE FROM sh_metrics m USING sh_startups s WHERE m.id=%s AND m.startup_id=s.id AND s.user_id=%s",(mid,g.uid))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def add_team(sid):
    ...
    if not row or row["user_id"]!=g.uid: cur.close(); return jsonify({"error":"Forbidden"}), 403
    d=request.get_json() or {}
    cur.execute("INSERT INTO sh_team(startup_id,name,role,bio,avatar_url,linkedin,twitter) VALUES(%s,%s,%s,%s,%s,%s,%s) RETURNING id",
...
def delete_team(tid):
    db=get_db(); cur=db.cursor()
    cur.execute("DELETE FROM sh_team t USING sh_startups s WHERE t.id=%s AND t.startup_id=s.id AND s.user_id=%s",(tid,g.uid))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def post_update(sid):
    ...
    cur.execute("INSERT INTO sh_updates(startup_id,user_id,title,content,type,is_pinned) VALUES(%s,%s,%s,%s,%s,%s) RETURNING id",
                (sid,g.uid,title,content,utype,d.get("is_pinned",False)))
    uid=cur.fetchone()["id"]
    cur.execute("UPDATE sh_startups SET last_update_post=NOW(),signal_score=signal_score+3,updated_at=NOW() WHERE id=%s",(sid,))
...
def delete_update(uid):
    db=get_db(); cur=db.cursor()
    cur.execute("DELETE FROM sh_updates u USING sh_startups s WHERE u.id=%s AND u.startup_id=s.id AND s.user_id=%s",(uid,g.uid))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def send_startup_message(sid):
    ...
    db=get_db(); cur=db.cursor()
    cur.execute("SELECT user_id,name FROM sh_startups WHERE id=%s AND is_active=TRUE",(sid,))
    s=cur.fetchone()
    if not s: cur.close(); return jsonify({"error":"Not found"}), 404
    sname=d.get("name",""); semail=d.get("email","")
    if uid:
        cur.execute("SELECT name,email FROM sh_users WHERE id=%s",(uid,))
        u=cur.fetchone()
    ...
    if uid and uid!=s["user_id"]:
        cur.execute("""SELECT c.id FROM sh_conversations c
                       JOIN sh_conversation_members m1 ON c.id=m1.conversation_id AND m1.user_id=%s
                       JOIN sh_conversation_members m2 ON c.id=m2.conversation_id AND m2.user_id=%s""",(uid,s["user_id"]))
        ex=cur.fetchone()
        if ex: conv_id=ex["id"]
        else:
            cur.execute("INSERT INTO sh_conversations DEFAULT VALUES RETURNING id"); conv_id=cur.fetchone()["id"]
            cur.execute("INSERT INTO sh_conversation_members(conversation_id,user_id) VALUES(%s,%s),(%s,%s)",(conv_id,uid,conv_id,s["user_id"]))
        cur.execute("INSERT INTO sh_messages(conversation_id,sender_id,content) VALUES(%s,%s,%s)",(conv_id,uid,content))
        notify(s["user_id"],uid,"startup_message",sid,message=f"New message about {s['name']}")
    cur.execute("UPDATE sh_startups SET message_count=message_count+1,signal_score=signal_score+1 WHERE id=%s",(sid,))
...
def get_startup_messages(sid):
    ...
    cur.execute("""SELECT m.*,u.avatar_url AS sender_avatar,u.name AS sender_name_db
                   FROM sh_messages m LEFT JOIN sh_users u ON m.sender_id=u.id
                   WHERE m.conversation_id IN (
                       SELECT DISTINCT cm.conversation_id FROM sh_conversation_members cm WHERE cm.user_id=%s
...
def get_conversations():
    ...
    cur.execute("""SELECT c.id,u2.id AS partner_id,u2.name AS partner_name,u2.username AS partner_username,
                   ...
                   (SELECT COUNT(*) FROM sh_messages WHERE conversation_id=c.id AND sender_id!=%s AND is_read=FALSE) AS unread
                   FROM sh_conversations c
                   JOIN sh_conversation_members cm1 ON c.id=cm1.conversation_id AND cm1.user_id=%s
                   JOIN sh_conversation_members cm2 ON c.id=cm2.conversation_id AND cm2.user_id!=%s
                   JOIN sh_users u2 ON cm2.user_id=u2.id
                   LEFT JOIN LATERAL (SELECT content,created_at FROM sh_messages WHERE conversation_id=c.id ORDER BY created_at DESC LIMIT 1) lm ON TRUE
...
def start_conversation():
    ...
    cur.execute("""SELECT c.id FROM sh_conversations c
                   JOIN sh_conversation_members m1 ON c.id=m1.conversation_id AND m1.user_id=%s
                   JOIN sh_conversation_members m2 ON c.id=m2.conversation_id AND m2.user_id=%s""",(g.uid,other_id))
    ex=cur.fetchone()
    if ex: cur.close(); return jsonify({"conversation_id":ex["id"]})
    cur.execute("INSERT INTO sh_conversations DEFAULT VALUES RETURNING id"); conv_id=cur.fetchone()["id"]
    cur.execute("INSERT INTO sh_conversation_members(conversation_id,user_id) VALUES(%s,%s),(%s,%s)",(conv_id,g.uid,conv_id,other_id))
...
def get_conv_messages(cid):
    db=get_db(); cur=db.cursor()
    cur.execute("SELECT 1 FROM sh_conversation_members WHERE conversation_id=%s AND user_id=%s",(cid,g.uid))
    ...
    cur.execute("""SELECT m.*,u.name AS sender_name,u.avatar_url AS sender_avatar
                   FROM sh_messages m JOIN sh_users u ON m.sender_id=u.id
                   WHERE m.conversation_id=%s ORDER BY m.created_at ASC""",(cid,))
    msgs=cur.fetchall()
    cur.execute("UPDATE sh_messages SET is_read=TRUE WHERE conversation_id=%s AND sender_id!=%s",(cid,g.uid))
    cur.execute("UPDATE sh_conversation_members SET last_read_at=NOW() WHERE conversation_id=%s AND user_id=%s",(cid,g.uid))
    db.commit(); cur.close()
...
def send_message(cid):
    ...
    cur.execute("SELECT 1 FROM sh_conversation_members WHERE conversation_id=%s AND user_id=%s",(cid,g.uid))
    if not cur.fetchone(): cur.close(); return jsonify({"error":"Forbidden"}), 403
    cur.execute("INSERT INTO sh_messages(conversation_id,sender_id,content) VALUES(%s,%s,%s) RETURNING id,created_at",(cid,g.uid,content))
    row=cur.fetchone()
    cur.execute("UPDATE sh_conversations SET updated_at=NOW() WHERE id=%s",(cid,))
    db.commit(); cur.close()
...
def check_messages(cid):
    ...
    cur.execute("SELECT COUNT(*) AS c FROM sh_messages WHERE conversation_id=%s",(cid,))
    total=cur.fetchone()["c"]
    cur.execute("SELECT last_active FROM sh_users u JOIN sh_conversation_members cm ON cm.user_id=u.id WHERE cm.conversation_id=%s AND cm.user_id!=%s",(cid,g.uid))
...
def community_posts():
    ...
    cur.execute(f"""SELECT cp.*,u.name AS author_name,u.username AS author_username,u.avatar_url AS author_avatar
                    FROM sh_community_posts cp JOIN sh_users u ON cp.user_id=u.id
                    {where} ORDER BY {order} LIMIT %s OFFSET %s""",params+[per_page,offset])
    rows=cur.fetchall()
    cur.execute(f"SELECT COUNT(*) AS t FROM sh_community_posts cp {where}",params)
    total=cur.fetchone()["t"]
    user_votes={}
    if g.uid and rows:
        ids=[r["id"] for r in rows]
        cur.execute("SELECT post_id,vote_type FROM sh_community_votes WHERE user_id=%s AND post_id=ANY(%s)",(g.uid,ids))
...
def create_post():
    ...
    db=get_db(); cur=db.cursor()
    cur.execute("INSERT INTO sh_community_posts(user_id,community,title,content,media_url,media_type) VALUES(%s,%s,%s,%s,%s,%s) RETURNING id",
...
def vote_post(pid):
    ...
    db=get_db(); cur=db.cursor()
    cur.execute("SELECT vote_type FROM sh_community_votes WHERE post_id=%s AND user_id=%s",(pid,g.uid))
    existing=cur.fetchone()
    if existing:
        if existing["vote_type"]==vtype:
            cur.execute("DELETE FROM sh_community_votes WHERE post_id=%s AND user_id=%s",(pid,g.uid))
            cur.execute(f"UPDATE sh_community_posts SET {vtype}vote_count=GREATEST(0,{vtype}vote_count-1) WHERE id=%s",(pid,))
            db.commit(); cur.close(); return jsonify({"vote_type":None})
        else:
            cur.execute("UPDATE sh_community_votes SET vote_type=%s WHERE post_id=%s AND user_id=%s",(vtype,pid,g.uid))
            old={"up":"down","down":"up"}[vtype]
            cur.execute(f"UPDATE sh_community_posts SET {vtype}vote_count={vtype}vote_count+1,{old}vote_count=GREATEST(0,{old}vote_count-1) WHERE id=%s",(pid,))
    else:
        cur.execute("INSERT INTO sh_community_votes(post_id,user_id,vote_type) VALUES(%s,%s,%s)",(pid,g.uid,vtype))
        cur.execute(f"UPDATE sh_community_posts SET {vtype}vote_count={vtype}vote_count+1 WHERE id=%s",(pid,))
...
def get_comments(pid):
    ...
    cur.execute("""SELECT cc.*,u.name AS author_name,u.username AS author_username,u.avatar_url AS author_avatar
                   FROM sh_community_comments cc JOIN sh_users u ON cc.user_id=u.id
...
def post_comment(pid):
    ...
    if not content: return jsonify({"error":"Content required"}), 400
    db=get_db(); cur=db.cursor()
    cur.execute("INSERT INTO sh_community_comments(post_id,user_id,parent_id,content) VALUES(%s,%s,%s,%s) RETURNING id",
                (pid,g.uid,d.get("parent_id"),content))
    cid=cur.fetchone()["id"]
    cur.execute("UPDATE sh_community_posts SET comment_count=comment_count+1 WHERE id=%s",(pid,))
...
def get_notifications():
    ...
    cur.execute("""SELECT n.*,u.name AS actor_name,u.avatar_url AS actor_avatar
                   FROM sh_notifications n LEFT JOIN sh_users u ON n.actor_id=u.id
                   WHERE n.user_id=%s ORDER BY n.created_at DESC LIMIT 50""",(g.uid,))
    rows=cur.fetchall()
    cur.execute("SELECT COUNT(*) AS c FROM sh_notifications WHERE user_id=%s AND is_read=FALSE",(g.uid,))
...
def mark_notif_read():
    ...
    db=get_db(); cur=db.cursor()
    if nid: cur.execute("UPDATE sh_notifications SET is_read=TRUE WHERE id=%s AND user_id=%s",(nid,g.uid))
    else: cur.execute("UPDATE sh_notifications SET is_read=TRUE WHERE user_id=%s",(g.uid,))
    db.commit(); cur.close(); return jsonify({"success":True})
...
def dashboard():
    ...
    cur.execute("""SELECT id,name,slug,logo_url,stage,status,is_active,is_featured,
                   ...
                   FROM sh_startups WHERE user_id=%s ORDER BY created_at DESC""",(g.uid,))
    startups=cur.fetchall()
    ids=[s["id"] for s in startups]; unread=0
    if ids:
        cur.execute("SELECT COUNT(*) AS c FROM sh_messages m JOIN sh_conversation_members cm ON m.conversation_id=cm.conversation_id WHERE cm.user_id=%s AND m.sender_id!=%s AND m.is_read=FALSE",(g.uid,g.uid))
...
def startup_health(sid):
    ...
    cur.execute("SELECT user_id FROM sh_startups WHERE id=%s",(sid,)); row=cur.fetchone()
    if not row or row["user_id"]!=g.uid: cur.close(); return jsonify({"error":"Forbidden"}), 403
    cur.execute("SELECT * FROM sh_startups WHERE id=%s",(sid,)); s=cur.fetchone()
    cur.execute("SELECT * FROM sh_startup_content WHERE startup_id=%s",(sid,)); c=cur.fetchone()
    cur.execute("SELECT COUNT(*) AS c FROM sh_metrics WHERE startup_id=%s AND is_current=TRUE",(sid,)); metrics_count=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_team WHERE startup_id=%s",(sid,)); team_count=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_updates WHERE startup_id=%s",(sid,)); updates_count=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_asks WHERE startup_id=%s AND is_active=TRUE",(sid,)); asks_count=cur.fetchone()["c"]
    # Platform averages for comparison
    cur.execute("SELECT AVG(upvote_count) AS avg_uv,AVG(view_count) AS avg_vw FROM sh_startups WHERE is_active=TRUE"); platform=cur.fetchone()
    cur.close()
...
def search():
    ...
    if tab in ("startups","all"):
        cur.execute("SELECT id,name,slug,tagline,logo_url,categories,stage,upvote_count,verification_level FROM sh_startups WHERE is_active=TRUE AND (name ILIKE %s OR tagline ILIKE %s OR %s=ANY(categories)) ORDER BY upvote_count DESC LIMIT 12",(lq,lq,q))
        startups=cur.fetchall()
    else: startups=[]
    if tab in ("users","all"):
        cur.execute("SELECT id,name,username,avatar_url,headline,is_verified FROM sh_users WHERE name ILIKE %s OR username ILIKE %s LIMIT 8",(lq,lq))
        users=cur.fetchall()
    else: users=[]
    if tab in ("posts","all"):
        cur.execute("SELECT cp.id,cp.community,cp.title,cp.content,cp.upvote_count,u.name AS author_name FROM sh_community_posts cp JOIN sh_users u ON cp.user_id=u.id WHERE cp.title ILIKE %s OR cp.content ILIKE %s ORDER BY cp.upvote_count DESC LIMIT 8",(lq,lq))
        posts=cur.fetchall()
    else: posts=[]
    cur.close()
...
def platform_stats():
    db=get_db(); cur=db.cursor()
    cur.execute("SELECT COUNT(*) AS c FROM sh_startups WHERE is_active=TRUE"); ts=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_users"); tu=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_startups WHERE is_active=TRUE AND created_at>=NOW()-INTERVAL '7 days'"); nw=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_upvotes WHERE created_at>=CURRENT_DATE"); ud=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_signals WHERE created_at>=NOW()-INTERVAL '7 days'"); sw=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_startups WHERE verification_level IN('verified','audited') AND is_active=TRUE"); vc=cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM sh_community_posts WHERE created_at>=NOW()-INTERVAL '24 hours'"); cp=cur.fetchone()["c"]
    cur.close()
...
def activity_feed():
    ...
    cur.execute("""SELECT a.type,a.meta,a.created_at,u.name AS user_name,u.username AS user_username,u.avatar_url AS user_avatar,
                   ...
                   FROM sh_activity a LEFT JOIN sh_users u ON a.user_id=u.id
                   LEFT JOIN sh_startups s ON a.startup_id=s.id
...
def get_user(username):
    db=get_db(); cur=db.cursor()
    cur.execute("SELECT id,name,username,avatar_url,cover_url,bio,headline,website,twitter,linkedin,github,is_verified,reputation,last_active,created_at FROM sh_users WHERE username=%s",(username,))
    u=cur.fetchone()
    if not u: cur.close(); return jsonify({"error":"Not found"}), 404
    cur.execute("SELECT id,name,slug,tagline,logo_url,categories,stage,status,upvote_count,view_count,is_active,created_at FROM sh_startups WHERE user_id=%s AND is_active=TRUE ORDER BY created_at DESC",(u["id"],))
